...
formula_generator1(bool, X50, X51, X30, X37, X56, X65, X34, X15, 
                   X62, X55) :- !,
    X31            = [],                                        
    X1 = [],                                        
    X38              = [],                                        
    ...
    (get_option(cluster,           X51, [[X57],X58]) ->         
        X15 = [cluster([X57],X58)|X12]       
    ;                                                                   
    ...
    memberchk(conds(X59), X15),
    pos_bool_cond_entry(lcondattr,      X26),
    pos_bool_cond_entry(lcondextravars, X10),
    get_entries_of_all_boolean_conditions(X59, X26,      X40),
    get_entries_of_all_boolean_conditions(X59, X10, X16),
    flatten(X40, X24),                                  
    flatten(X16, X9),                        
    append(X24, X9, X46),                
    length(X22, X63),
    ...
    gen_occ_vars(2, X60, X17, X52, _),      
    length(X24, X11),
    X53 is X11-X63,
    X64 in 0..X53,                                                 
    global_cardinality(X24, [1-X64|X52]),                
                 


    instrument_code(instrument_in_formula_generator, X50),
    search_a_single_formula_for_all_table_entries(X50, X15, col(X56,X65)),
    X54 = 12000000,                                                   
    (get_option(then_else,         X51, X45) ->                
        X45 = [then(X32, X47, X41, X18, _,
                    ...
                    _, _, _, _)],
        formula_pattern(op(X61),                 X15),
        avoid_simplifiable_conditional(X59, X22, X61,
                                        X32, X33,
                                        X41,   X42,
                                        X47,    X48),
        append([X31,X41,X42],       X27),
        append([X46,X18,X20],X43),
        (user:max_cost(X66) -> X62 in 0..X66 ; X66 = X34, X62 in 0..X66),
        ...
        time_out(minimize(label(X27, X1,
                                X43, X38), X62, [all]), X54, X55)
    ;
        (user:max_cost(X66) -> X62 in 0..X66 ; X66 = X34, X62 in 0..X66),
        get_bool_cost_vars(X15, X28),                   
        gen_cost_sum_abs(X28, X62),
        time_out(minimize(label(X31, X1, X46, X38), X62, [all]), X54, X55)
    ).
...
search_a_single_formula_for_all_table_entries_bool(_, X5, X19) :-
    formula_pattern(mandatory_attr(X12),          X5),      
    formula_pattern(neg(X17),                       X5),      
                                                                              
    formula_pattern(shift(X13),                   X5),      
                                                                              
    formula_pattern(op(X20),                          X5),      
    formula_pattern(nb_terms(X18),                  X5),      
    formula_pattern(conds(X21),                       X5),      
    formula_pattern(combined_row_ctrs(X4,                        
                                      X2,
                                      X3,
                                      X9),      X5),
    X19 = col(X22,_),                                                    
    tab_get_arity(col(X22,_), X23),                                       
    append(X12, [X19], X7),                                
    length(X7, X14),                                          
    length(X10, X14),                                            
    functor(X11, X22, X23),                                        
    build_source_target_extraction_terms(X7, X11, X10),
    (formula_pattern(cluster(X24,X25), X5) ->                 
        findall(X1, (call(user:X11),                    
                                    update_table_entry_cluster(X10,    
                                                               X24,         
                                                               X25,         
                                                               X1)),
                                   X8)
    ;                                                                         
        findall(X10, call(user:X11),                            
                            X8)                                     
    ),
    sort(X8,X6),
    (formula_pattern(then_else(X16), X5) ->
        true
    ;
        X16 = []
    ),
    post_bool_on_all_table_entries(X6, X12, X17, X13, X20, X18, X21, 
                                   combined_row_ctrs(X4, X2, X3, X9),
                                   X16).
