...
protected_correlation(X1, X2, X3) :- 
    ((more_than_one_unique_value(X1),
      more_than_one_unique_value(X2)) ->
        correlation(X1, X2, X3)
    ;
        X3 is 0
    ).

get_correlation_wrt_test(X2, X1, X3) :-
    X4 is abs(X2),
    (X4 > X1 ->        
        X3 is -X4            
    ;                                               
...
f_multiple_correlation(X2, X3, X4, X5) :-
    ((X2 >= 0.99999, X2 < 1.00001) ->
        X5 is 1000                                
    ;
        X1 is X3 - X4 - 1,
        (X1 =\= 0 ->
                X5 is (X2 / X4) / ((1 - X2) / (X3 - X4 - 1))
        ; 
                X5 is 0                           
        )
    ).

adjusted_multiple_correlation(X5, X6, X3, X1) :-
    multiple_correlation(X5, X6, X4),
    X1 is X4*X4, 
    X6 = [X7|_],
    length(X7, X10),
    length(X5, X11),
...
f_distribution(X1, X2, X3) :-
    (X1 =<  10 -> X6 is max(1, X1) ;
     X1 =<  12 -> X6 is 11                          ;
     X1 =<  15 -> X6 is 12                          ;
     X1 =<  20 -> X6 is 13                          ;
     X1 =<  24 -> X6 is 14                          ;
...
critical_vals_for_pearson_correlation_coef(X3, X2) :-
    X4 is min(max(1, X3-2), 100),
    r_table(X1),
    memberchk([X4, X2], X1).

multiple_correlation(X3, X5, X1) :-
    X3 = [_|_],                                                         
    same_length(X3, X5),
    transpose(X5, X4),                                         
    X2 = [X3|X4],                              
    ...
    findall(X8, (X18 in 1..X17, indomain(X18),
                      length(X8, X17), nth1(X18, X6, X8),
                      findall(X11, (X19 in 1..X17, indomain(X19),
                                     (X18 = X19 -> X11 is 1                        ; 
                                      X19 > X18 -> nth1(X18, X2, X12),  
                                               nth1(X19, X2, X13),  
                                               protected_correlation(X12, X13, X11)
                                    ;
                                     X11 = _                                   ) 
                                    ), X8)), X6),
    ...
    findall(X8, (X18 in 1..X17, indomain(X18),
                      ...
                      findall(X11, (X19 in 1..X17, indomain(X19),
                                    ...
                                    ;
                                      nth1(X19, X6, X14),
                                      nth1(X18, X14, X11))), X8)), X7),

    
    determinant(X7, X9),

    
    co_factor(1, 1, X7, X15),
    determinant(X15, X10),

    
    ((X10 >= -0.00001, X10 =< 0.00001) ->
        X1 = 0
    ;
        X1 is min(1,sqrt(1 - (X9 / X10)))
    ).
...
determinant(X2, X6) :-
    ...
    X2 = [X7|_],                                   
    switch_signs(X7, X3),                          
    findall(X4, (X10 in 1..X9, indomain(X10),              
                   co_factor(1, X10, X2, X1),   
                   determinant(X1, X4)), X5),
    scalar_prod(X5, X3, X6).                     

co_factor(X8, X9, X2, X1) :-
    length(X2, X10),
    findall(X5, (X6 in 1..X10, indomain(X6),                                     
                  X6 =\= X8, nth1(X6, X2, X3),                             
                  findall(X4, (X7 in 1..X10, indomain(X7),                      
                                 X7 =\= X9, nth1(X7, X3, X4)), X5)          
                  ), X1).

switch_signs([], []) :- !.
switch_signs([X1|X2], [X1|X3]) :-
    switch_signs1(X2, X3).

switch_signs1([], []) :- !.
switch_signs1([X2|X3], [X1|X4]) :-
    X1 is -X2,
    switch_signs(X3, X4).
