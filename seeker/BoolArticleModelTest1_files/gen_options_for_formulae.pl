...
gen_options_lists(X6, col(X9,X7), X10, X2, X3, X5, X1, X4, X8) :- 
    (X6 \== model_seeker ->
        gen_options_lists_by_type(bool,    X6, col(X9,X7), X10, X2, X3, X5, X1, X4),
        gen_value_sets_wrt_boolean_operators(X6, X5, col(X9,X7), [], [], X8)
    ;
        gen_options_lists_by_type(bool,    X6, col(X9,X7), X10, X2, X3, X5, X1, X4),
        ((X4 = []) ->
            X8 = []
...
gen_options_lists_by_type(cluster, _, col(X6,X5), _, _, _, _, _, []) :- 
    ...
    !.                                               
gen_options_lists_by_type(X16, X14, col(X21,X15), X24, X4, X5, X9, X1, X7) :-
    memberchk(X16,[bool,cond,cond_ex]),         
    !,                                               
    (X1 = 1 ->                    
        X12 = [mandatory_attr(X9)]   
    ;                                                
        gen_filtered_fd(X14, col(X21,X15), X24, X5, X9, X12)
    ),
    X8 = [[attr, coef(X27,X28)],
                    [coef(X27,X28), attr],
                    [coef(X27,X28),coef(X27,X28)]], 
    (X12 = [] ->
        X7 = []
    ;
        (X16 = bool ->                         
            ...
            X4 = X10-_,        
            call(user:table_info(X21,_,_,X18,X22,X19)),
            memberchk([bool([neg(_), op(_), nb_terms(_,_), conds(X23), use_mods(_)])], X10),
            X6 = [[bool([neg(X18), op([X22]), nb_terms(X19,X19), conds(X23), use_mods(2)])]]
        ;
         X16 = cond_ex ->                         
            X4 = X10-X11, 
            option_cond_min_cst(X14, X27), 
            option_cond_max_cst(X14, X28), 
            findall(X13,
                    (member(X13,X11),
                     X13 = [_,then(X25),else(X26)],
                     member([X25, X26],
                            X8)
                    ),
                    X3),
            findall([cond_ex([then(X25), else(X26), bool([neg(X17), op(X31), nb_terms(X29,X32), conds(X23), use_mods(X20)])])],
                    (member([bool([neg(X17), op(X31), nb_terms(X29,X32), conds(X23), use_mods(X20)])], X10),
                     X17 = 0,             
                     X31 = [X30],
                     memberchk(X30,[and,or]),
                     X29 >= 2,
                     X20 = 0,
                     member([X25, X26],
                            X8)
